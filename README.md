...
| **Solution generator** (`lib/solutions/generate.ts`) | ⚠️ Calls the live Anthropic API (`ANTHROPIC_API_KEY`) when configured — falls back to a deterministic template otherwise, or if the live call fails. See [§8 of architecture.md](./architecture.md#8-ai-semantic-analyzer--the-llm-seam) and `lib/solutions/generate.ts`. |
